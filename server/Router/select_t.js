...
router.post("/matchinglist", (req, res) => {
    database.query(
        "INSERT INTO select_t(t_name, curri, face, area, day, time, free) values (?, ?, ?, ?, ?, ?, ?)", [req.body.t_name, req.body.curri ,req.body.face, req.body.area, req.body.day, req.body.time, req.body.free],
    function(err, data){
        if(err){
            console.log(err);
        } else{
